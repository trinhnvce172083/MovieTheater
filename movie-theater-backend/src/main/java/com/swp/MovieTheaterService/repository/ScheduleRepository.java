...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
       ...
       List<Schedule> findByMovieMovieIdAndShowDateBetweenAndIsActiveTrue(Long movieId, LocalDate startDate,
                     LocalDate endDate);
       ...
       List<Schedule> findByCinemaRoomCinemaRoomIdAndShowDateBetweenAndIsActiveTrue(Long roomId, LocalDate startDate,
                     LocalDate endDate);
       ...
       // Check for overlapping schedules - HỖ TRỢ PHIM QUA ĐÊM
       @Query("SELECT s FROM Schedule s WHERE s.cinemaRoom.cinemaRoomId = :cinemaRoomId AND " +
                     ...
                     "    (s.startTime >= :startTime AND s.endTime <= :endTime))) OR " +
                     "  (s.showDate = :showDate AND s.startTime > s.endTime AND " +
                     "   (s.startTime <= :startTime OR s.endTime >= :endTime))" +
                     ")")
