...
export default function OrderSnacksPage() {
  ...
  const total = snacks.reduce((sum, item, index) => sum + item.price * quantities[index], 0);
    ...
    <main className="min-h-screen bg-gradient-to-b from-[#0e0d20] to-black text-white">
      <div className="max-w-7xl mx-auto p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section className="lg:col-span-2 bg-black bg-opacity-40 rounded-lg p-6">
          <div className="flex justify-between border-b border-gray-700 pb-2 font-semibold text-sm text-gray-300 uppercase">
            <span>Combo</span>
            <span>Price</span>
            <span>Quantity</span>
          </div>
          {snacks.map((item, index) => (
            <div key={item.id} className="flex justify-between items-center border-b border-gray-800 py-4">
              <div>
                <div className="text-white font-medium">{item.name}</div>
                {item.detail && <div className="text-sm text-gray-400">{item.detail}</div>}
              </div>
              <div className="text-gray-200">{item.price.toLocaleString()} VND</div>
              <div className="flex items-center space-x-2">
                ...
                <span className="w-5 text-center" aria-live="polite">{quantities[index]}</span>
                <button
                  className="w-6 h-6 flex items-center justify-center rounded bg-gray-700 hover:bg-gray-600"
                  onClick={() => handleQuantityChange(index, 1)}
        ...
        <aside className="bg-black bg-opacity-40 rounded-lg p-6">
          ...
          <p className="text-xl font-bold text-purple-400 mb-4" aria-live="polite">{total.toLocaleString()} VND</p>
          <div className="flex space-x-2">
            <button className="flex-1 py-2 bg-gray-700 hover:bg-gray-600 rounded text-white">←</button>
            <button className="flex-1 py-2 bg-purple-600 hover:bg-purple-700 rounded text-white font-semibold">Continue</button>
          </div>
        </aside>
      </div>
    </main>
