...
export default function ProfessionalPromotionManagement() {
  ...
  // Fetch data from API
  const fetchPromotions = async (page = 0, pageSize = 10, sortBy = "startDate", sortDirection = "desc", isActive = null) => {
    try {
      setLoading(true);
  ...
  const filteredData = useMemo(() => {
    return promotions.filter((promotion) => {
      const matchesSearch =
        ...
        (promotion.promotionId || "").toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const statistics = useMemo(() => {
    ...
    const activePromotions = promotions.filter(p => p.isActive && !p.isExpired).length;
    const pointsPromotions = promotions.filter(p => p.isPointsPromotion).length;
    ...
    const formData = {
      ...
      membershipLevels: record.membershipLevels ? record.membershipLevels.split(',').filter(Boolean) : [],
  ...
  const handleBulkDelete = async () => {
    try {
      const promises = selectedPromotions.map(promotion => 
        fetch(`http://localhost:8080/cinema/api/promotions/${promotion.promotionId}`, {
          method: "DELETE",
          headers: {
  ...
  const handleDelete = async (record: PromotionDto) => {
    ...
    try {      console.log("Starting form validation..."); // Debug log
      ...
      if (values.discountType === 'BUY_ONE_GET_ONE' && values.discountValue !== 1) {
      ...
      // Date fields
      if (values.startDate) {
        payload.startDate = values.startDate?.format ? values.startDate.format("YYYY-MM-DD") : values.startDate;
      }
      if (values.endDate) {
        payload.endDate = values.endDate?.format ? values.endDate.format("YYYY-MM-DD") : values.endDate;
      }
      
      // Optional numeric fields - only include if they have valid values
      if (values.minPurchase !== undefined && values.minPurchase !== null && values.minPurchase !== '') {
        payload.minPurchaseAmount = Number(values.minPurchase);
      } else {
        payload.minPurchaseAmount = 0; // Default value
      }
      
      if (values.maxDiscount !== undefined && values.maxDiscount !== null && values.maxDiscount !== '') {
        payload.maxDiscountAmount = Number(values.maxDiscount);
      }
      
      if (values.pointsRequired !== undefined && values.pointsRequired !== null && values.pointsRequired !== '') {
        payload.pointsRequired = Number(values.pointsRequired);
      }
      
      if (values.maxUsageCount !== undefined && values.maxUsageCount !== null && values.maxUsageCount !== '') {
        payload.maxUsageCount = Number(values.maxUsageCount);
      }
      
      if (values.maxUsagePerUser !== undefined && values.maxUsagePerUser !== null && values.maxUsagePerUser !== '') {
      ...
      // Array field
      if (values.membershipLevels && Array.isArray(values.membershipLevels) && values.membershipLevels.length > 0) {
        payload.membershipLevels = values.membershipLevels.filter(Boolean).join(",");
      }
      ...
      if (editingPromotion) {
        ...
        response = await fetch(`http://localhost:8080/cinema/api/promotions/${editingPromotion.promotionId}`, {
          method: "PUT",
          headers: {
        ...
        if (response.status === 403) {
          errorMessage = "Bạn không có quyền thực hiện thao tác này. Vui lòng liên hệ admin để được cấp quyền.";
      ...
      if (values.banner && values.banner.length > 0) {
        ...
        await fetch(`http://localhost:8080/cinema/api/promotions/${result.promotionId}/banner`, {
          method: "POST",
          headers: {
            "Authorization": `Bearer ${localStorage.getItem("accessToken")}`,
          },
  ...
  const columns: ColumnsType<PromotionDto> = [
    ...
    },
    {
      ...
      title: "Period",
      key: "period",
      width: 150,      render: (_: any, record: PromotionDto) => (
        <div className="text-sm">
      ...
      width: 80,
      align: "center" as const,      render: (count: number, record: PromotionDto) => (
        <div className="text-center">
    ...
    },
    {
      ...
      render: (_: any, record: PromotionDto) => (
        <div className="text-sm">
          <div className="text-gray-900 mb-1">
            {record.membershipDisplay || (record.memberOnly ? "Members Only" : "All Users")}
          </div>
      ...
      <div className="container mx-auto px-4 py-6 max-w-7xl">
        ...
        >
          ...
          <div className="bg-white">            <Table
              ...
              rowSelection={{
                selectedRowKeys,
                onChange: (selectedRowKeys: React.Key[], selectedRows: PromotionDto[]) => {
        ...
        >
          ...
          <Row gutter={16}>
            <Col xs={24} sm={12}>
              ...
              >
                <Input placeholder="Enter promotion name (e.g., Summer Sale)" className="h-10" />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12}>
              <Form.Item
          ...
          <Row gutter={16}>
            <Col xs={24}>
              ...
              >
                <Input.TextArea rows={3} placeholder="Enter promotion description (e.g., Get 20% off on all movie tickets this summer!)" />
              </Form.Item>
            </Col>
          </Row>
            ...
            <Col xs={24} sm={12}>
              ...
              >
                <InputNumber
                  ...
                  placeholder={
                    form.getFieldValue("discountType") === "PERCENTAGE" ? "Enter percentage (1-100)" :
                    form.getFieldValue("discountType") === "BUY_ONE_GET_ONE" ? "1" :
                    "Enter amount in VND"
                  }
                  disabled={form.getFieldValue("discountType") === "BUY_ONE_GET_ONE"}
                  value={form.getFieldValue("discountType") === "BUY_ONE_GET_ONE" ? 1 : undefined}
                  addonAfter={
                    form.getFieldValue("discountType") === "PERCENTAGE" ? "%" :
                    form.getFieldValue("discountType") === "FIXED_AMOUNT" ? "VND" : ""
                  }
                />
              </Form.Item>
            </Col>
          ...
          <Row gutter={16}>
            ...
            <Col xs={24} sm={12}>
              ...
              >
                <Select mode="multiple" placeholder="Select applicable membership levels" allowClear>
          ...
          <Row gutter={16}>
            <Col xs={24} sm={12}>
              ...
              >
                <InputNumber min={0} className="w-full" placeholder="Leave empty for unlimited" />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12}>
              <Form.Item
            ...
            <Col xs={24} sm={12}>
              ...
              >
                <InputNumber 
                  min={0} 
                  className="w-full" 
                  disabled={form.getFieldValue('promotionType') !== 'POINT_BASED'}
      ...
      >
        {viewingPromotion && (
          <div className="space-y-6">
              ...
              <Row gutter={[16, 16]}>
                <Col xs={24} sm={12}>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-600">Promotion Code</label>
                ...
                <Col xs={24} sm={12}>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-600">Promotion Name</label>
            ...
            {/* Discount Information */}
            <div className="border rounded-lg p-4">
              <h3 className="text-lg font-semibold mb-3 text-gray-800">Discount Information</h3>
              <Row gutter={[16, 16]}>
                <Col xs={24} sm={8}>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-600">Discount Type</label>
                    <div className="text-base bg-gray-50 p-2 rounded">
                      <Tag color={viewingPromotion.discountType === 'PERCENTAGE' ? 'blue' : 
                                  viewingPromotion.discountType === 'FIXED_AMOUNT' ? 'green' : 'purple'}>
                ...
                <Col xs={24} sm={8}>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-600">Min Purchase</label>
                    <div className="text-base bg-gray-50 p-2 rounded">
            ...
            {/* Validity & Usage */}
            <div className="border rounded-lg p-4">
              <h3 className="text-lg font-semibold mb-3 text-gray-800">Validity & Usage</h3>
              <Row gutter={[16, 16]}>
                <Col xs={24} sm={12}>
                  <div className="space-y-2">
                ...
                <Col xs={24} sm={12}>
                  <div className="space-y-2">
                    ...
                    <div className="text-base bg-gray-50 p-2 rounded">
                      <Tag color={viewingPromotion.isActive && !viewingPromotion.isExpired ? 'success' : 
                                  viewingPromotion.isExpired ? 'error' : 'warning'}>
                        {viewingPromotion.statusDisplay}
                      </Tag>
                      {viewingPromotion.isFeatured && (
                        <Tag color="orange" className="ml-2">Featured</Tag>
                      )}
                    </div>
                  </div>
                </Col>
            ...
            {viewingPromotion.isPointsPromotion && (
              <div className="border rounded-lg p-4">
                <h3 className="text-lg font-semibold mb-3 text-gray-800">Points Information</h3>
                <Row gutter={[16, 16]}>
                  <Col xs={24} sm={12}>
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-gray-600">Points Required</label>
                      <div className="text-base bg-gray-50 p-2 rounded">
                  ...
                  <Col xs={24} sm={12}>
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-gray-600">Code Validity</label>
                      <div className="text-base bg-gray-50 p-2 rounded">
            ...
            {/* Applicability */}
            <div className="border rounded-lg p-4">
              <h3 className="text-lg font-semibold mb-3 text-gray-800">Applicability</h3>
              <Row gutter={[16, 16]}>
                <Col xs={24} sm={12}>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-600">Promotion Type</label>
                    <div className="text-base bg-gray-50 p-2 rounded">
                      <Tag color="cyan">{viewingPromotion.promotionTypeDisplay}</Tag>
                    </div>
                  </div>
                </Col>
                <Col xs={24} sm={12}>
            ...
            {viewingPromotion.bannerUrl && (
              <div className="border rounded-lg p-4">
                <h3 className="text-lg font-semibold mb-3 text-gray-800">Banner</h3>
                <img 
